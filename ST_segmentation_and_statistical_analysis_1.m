...
sample_value_begin=20;%%%%%for the correction of j point. Earlier it was 15
...
tangent_value_14=zeros(length(qrs_i_raw),1);
for z=1:length(qrs_i_raw)
    v=value(z,:);
    Slope_value(z,1)=abs((v(1,26)-v(1,1))/(k+1));% the value would be 31 for earlier case when the sample begin with 15
    tangent_value_14(z,1)=atand(Slope_value(z,1));
end
...
figure; bar(Slope_value); grid on; title('Slope value')
figure; bar(tangent_value_14); grid on; title('tangent value')

Mean_value_of_tangent=mean(tangent_value_14)
